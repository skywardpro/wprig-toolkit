	...
	sassFiles.forEach((sassFile) => {
		try {
			...
			while ((match = importRegex.exec(content)) !== null) {
				...
				const resolvedPath = path.resolve(path.dirname(sassFile), importPath);
...
const getAllSCSSFiles = (dir, sassImportedFiles = null) => {
	...
	files.forEach((file) => {
		...
		const fileStat = statSync(filePath);
		if (fileStat.isDirectory()) {
			filelist = filelist.concat(getAllSCSSFiles(filePath, sassImportedFiles));
		} else if (
...
const processCSSFile = async (filePath, outputPath, injectPreload = true) => {
	...
	if (isDev && result.map) {
		try {
			...
			if (!Array.isArray(mapJson.sources) || mapJson.sources.length === 0) {
				// eslint-disable-next-line no-console
				console.warn('[css] Warning: sourcemap has no sources for', filePath);
			}
		} catch (err) {
			// eslint-disable-next-line no-console
			console.warn('[css] Failed to parse sourcemap for', filePath, err);
		}
	}
...
(async () => {
	...
	await processDirectory(paths.styles.editorSrcDir, paths.styles.editorDest);
	...
	try {
		...
		for (const slug of slugs) {
			...
			if (existsSync(styleIn)) {
				await processCSSFile(styleIn, path.join(outDir, 'style.css'), false);
			}
			if (existsSync(editorIn)) {
				await processCSSFile(editorIn, path.join(outDir, 'editor.css'), false);
			}
		}
	} catch {
