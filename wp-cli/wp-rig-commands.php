...
class Rig_Command extends WP_CLI_Command
{
	...
	public function dev_setup(array $args, array $assoc_args): void
	{
		...
		$home_page_id = $this->get_or_create_page(
			'Home',
	...
	public function menu_import($args, $assoc_args)
	{
		...
		if ($dry_run) {
			...
			WP_CLI::log('Items to import: ' . count($menu_data['menu_items'] ?? []));

			$existing_menu = wp_get_nav_menu_object($menu_name);
			if ($existing_menu && !$overwrite) {
				WP_CLI::warning(
					"Menu '{$menu_name}' already exists. Use --overwrite to replace it.",
				);
			}
		...
		if ($existing_menu) {
			if (!$overwrite) {
				WP_CLI::error(
					"Menu '{$menu_name}' already exists. Use --overwrite flag to replace it.",
				);
			}
		...
		foreach ($menu_data['menu_items'] as $item_data) {
			$menu_item_args = [
				...
				'menu-item-classes' => is_array($item_data['classes'])
					? implode(' ', $item_data['classes'])
					: sanitize_text_field($item_data['classes']),
		...
		WP_CLI::success(
			"Menu '{$menu_name}' imported successfully with {$items_imported} items.",
		);
	}
	...
	public function menu_list($args, $assoc_args)
	{
		...
		WP_CLI\Utils\format_items($format, $menu_list, [
			'id',
			'name',
			'slug',
			'count',
	...
	public function fake_menu_items($args, $assoc_args)
	{
		...
		$subitems_count = (int) WP_CLI\Utils\get_flag_value(
			$assoc_args,
			'subitems',
			3,
		);
		...
		WP_CLI::log(
			sprintf(
				'Generating dummy menu items for menu "%s" (ID: %d)',
				$menu_name,
				$menu_id,
			),
		);
		...
		WP_CLI::success(
			sprintf(
				'Created %d top-level menu items in menu "%s"',
				$created_count,
				$menu_name,
			),
		);
	 ...
	 */
	private function create_submenu_items(
		$menu_id,
		$parent_id,
		$parent_prefix,
		$count,
	...
	{
		$font_dir = WP_CLI\Utils\get_flag_value(
			$assoc_args,
			'font-dir',
			'assets/fonts',
		);
		...
		$css_dir = WP_CLI\Utils\get_flag_value(
			$assoc_args,
			'css-dir',
			'assets/css/src',
		);
		...
		WP_CLI::success(
			sprintf('Google Fonts downloaded. CSS saved at: %s', $result),
		);
	}
}
