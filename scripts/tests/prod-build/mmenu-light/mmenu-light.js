...
!(function (t) {
	...
	function (t, e, s) {
		...
		const o = (t) => Array.prototype.slice.call(t),
			i = (t, e) => o((e || document).querySelectorAll(t));
		class a {
			constructor(t, e) {
				((this.options = Object.assign(a.options, e)),
					(this.menu = t),
			...
			_openPanel() {
				let t = i('.' + this.options.selected, this.menu),
					...
					s = null;
				(e && (s = e.closest('ul')),
					s || (s = this.menu.querySelector('ul')),
					this.openPanel(s));
			}
			openPanel(t) {
					...
					e = (t) => {
						...
						if (e.matches('.mm')) {
							const s = i('.mm--open', e),
								n = s[s.length - 1];
							if (n) {
								const t = n.parentElement.closest('ul');
								t && this.openPanel(t);
							}
		...
		const m = 'ontouchstart' in window || !!navigator.msMaxTouchPoints || !1;
		...
		a.prototype.offcanvas = function (t) {
			if (
				((t = Object.assign(a.optionsOffcanvas, t)),
				...
				'right' == t.position && this.menu.classList.add('mm--right'),
				t.move)
			) {
				let t;
				this.toggler.add(
			...
			t.blockPage &&
				...
				'right' == t.position && this.blocker.classList.add('mm--right'),
