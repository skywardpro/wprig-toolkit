...
test('getAssetPath maps assets to src directory', () => {
	expect(getAssetPath('assets/css/main.css')).toBe('assets/css/src/main.css');
	expect(getAssetPath('assets/js/main.js')).toBe('assets/js/src/main.js');
...
test('backslashToForwardSlash converts backslashes', () => {
	expect(backslashToForwardSlash('path\\to\\file')).toBe('path/to/file');
	expect(backslashToForwardSlash(['a\\b', 'c\\d'])).toEqual(['a/b', 'c/d']);
});

test('appendBaseToFilePathArray appends base path', () => {
	expect(appendBaseToFilePathArray('file.js', 'base')).toBe('base/file.js');
...
test('getReplacements protects block namespaces and block classes', () => {
	...
	expect(testBlockName.replace(searchValue, replaceValue)).toBe(testBlockName); // Should remain unchanged!
});
