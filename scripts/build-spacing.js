...
const processSpacingSystem = async () => {
	const scssFile = path.resolve(
		paths.styles.srcDir,
		'spacing-system/_spacing-system.scss'
	);
	const tempCssFile = path.resolve(
		paths.styles.srcDir,
		'spacing-system/_spacing-system.css'
	);
...
`;
	...
	console.log(
		`[Spacing System] Processing with LightningCSS ${isDev ? '(dev)' : '(production)'}...`
	);
