...
export default async function setupAgents(options = {}) {
	...
	console.log(colors.blue(`Optimizing for: ${selectedAgents.join(', ')}...`));
		...
		for (const file of agent.files) {
			...
			try {
				if (fs.existsSync(sourcePath)) {
					...
					console.log(
						colors.green(`✓ Created ${file.target} for ${agent.name}`)
					);
				} else {
					console.error(
						colors.red(
	...
	try {
		...
		console.log(
			colors.green('✓ Playwright Chromium browser installed successfully!')
		);
	} catch (error) {
		console.error(
			colors.yellow(
