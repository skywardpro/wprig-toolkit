...
export default async function testComponent(themeRoot, componentSlug) {
				...
				for (const type in manifest.asset_mapping) {
					...
					if (asset.src) {
						...
						if (await fs.pathExists(assetPath)) {
							console.log(c.green(`✓ Asset ${asset.src} exists`));
						} else {
							console.error(
								c.red(`✗ Asset ${asset.src} is missing from ${assetPath}`)
							);
							errors++;
						}
					}
	...
	if (await fs.pathExists(phpPath)) {
		...
		if (match) {
			...
			console.error(
				c.red(`✗ Component.php: could not find valid WP_Rig namespace`)
			);
			errors++;
		}
		...
		if (!content.includes('implements Component_Interface')) {
			console.error(
				c.red('✗ Component.php: class does not implement Component_Interface')
			);
			errors++;
		}
		...
		for (const pattern of dangerousPatterns) {
			if (pattern.regex.test(content)) {
				console.error(
					c.red(`✗ Component.php: Dangerous function detected: ${pattern.name}`)
				);
				errors++;
			}
		}
