...
export default async function prepareComponent(slug, themeRoot) {
	...
	logger.info(`Preparing component "${normalizedSlug}" for submission...`);
...
async function copyCoreFiles(componentDir, distDir) {
	const coreFiles = ['Component.php', 'manifest.json', 'SPEC.md', 'SKILL.md'];
...
function printNextSteps(realSlug, distDir) {
	...
	logger.log(
		c.gray('   git clone https://github.com/YOUR_USERNAME/wprig-components.git')
	);
	logger.log(c.gray('   cd wprig-components'));
	logger.log(
		c.gray(
			`   git checkout -b add-${realSlug.toLowerCase().replace(/_/g, '-')}`
		)
	);
	logger.log(
	...
	logger.log(
		c.gray(
			`   git push origin add-${realSlug.toLowerCase().replace(/_/g, '-')}`
		)
	);
	logger.log(
