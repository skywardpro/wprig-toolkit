 ...
 */
export default async function downloadComponent(slug, themeRoot, options = {}) {
	...
	logger.info(`${actionText} component "${slug}" from ${sourceText}...`);
	...
	try {
		...
		// Save manifest.json
		await fs.writeJson(path.join(componentDir, 'manifest.json'), component, {
			spaces: 2,
		});
...
async function checkVersions(componentDir, component) {
	...
	try {
		...
		if (
			...
			logger.info(`Component is already at version ${component.version}.`);
		} else if (component.version && localManifest.version > component.version) {
			logger.warn(
...
async function processAssets(component, themeRoot, componentDir, options) {
	...
	for (const type in component.asset_mapping) {
		...
		try {
			...
			const destPath = path.resolve(themeRoot, getAssetPath(asset.src));

			// Security check: Ensure destPath is within themeRoot.
			const relativeDestPath = path.relative(resolvedThemeRoot, destPath);
			if (
			...
			if (saved) {
				logger.success(`Downloaded asset to ${getAssetPath(asset.src)}`);
			}
		} catch (assetError) {
			logger.warn(`Error fetching asset ${asset.src}: ${assetError.message}`);
		}
	}
}
	...
	if (await fs.pathExists(filePath)) {
		if (options.forceOverwrite) {
			...
			if (existingContent !== content) {
				if (options.yes) {
					logger.warn(`Overwriting ${fileName} (changes detected, --yes used)`);
				} else {
					const { confirm } = await inquirer.prompt([
						{
							type: 'confirm',
...
async function installDependencies(component, themeRoot) {
	...
	if (component.npm_dependencies) {
		...
		if (deps.length > 0) {
			...
			logger.info(`Installing npm dependencies: ${safeDeps.join(', ')}`);
			try {
				runSafeCommand(
					'npm',
	...
	if (component.composer_dependencies) {
		...
		if (deps.length > 0) {
			...
			logger.info(`Installing composer dependencies: ${safeDeps.join(', ')}`);
			try {
				runSafeCommand('composer', ['require', ...safeDeps], themeRoot);
			} catch (e) {
				logger.error(`Failed to install composer dependencies: ${e.message}`);
			}
		}
	}
...
function sanitizeDependencyList(dependencies, source) {
	...
	return dependencies.map((dependency) => {
		...
		if ('' === normalizedDependency || !pattern.test(normalizedDependency)) {
			throw new Error(`Invalid ${source} dependency value: ${dependency}`);
		}
...
async function setupAiSkills(rawSlug, componentDir, themeRoot) {
	...
	for (const file of ['SPEC.md', 'SKILL.md']) {
		...
		try {
			...
			logger.warn(`Copied ${file} to .ai/skills/${rawSlug}/ (symlink failed)`);
		}
	}
}
