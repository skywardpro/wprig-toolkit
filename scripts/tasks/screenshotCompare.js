...
export default async function screenshotCompare(themeRoot, options = {}) {
	...
	try {
		...
		if (img1.width !== img2.width || img1.height !== img2.height) {
			...
			logger.info('Resizing live screenshot to match mockup dimensions...');
		...
		const fidelityScore = ((totalPixels - numDiffPixels) / totalPixels) * 100;
		...
		if (fidelityScore >= 98) {
			...
			logger.warn('⚠ Moderate fidelity. Some visible discrepancies found.');
		} else {
			logger.error(
