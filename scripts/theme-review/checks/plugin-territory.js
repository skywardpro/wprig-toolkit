	...
	const patterns = [
		{
			...
			message: 'Custom Post Types must be registered in a plugin, not a theme.',
			severity: REQUIRED,
		},
		{
			regex: /\bregister_taxonomy\s*\(/g,
			id: 'plugin-territory.taxonomy',
			message: 'Custom Taxonomies must be registered in a plugin, not a theme.',
			severity: REQUIRED,
		},
		{
		...
		{
			regex:
				/\b(register_block_type|register_block_type_from_metadata|wp_register_block_types_from_metadata_collection)\s*\(/g,
		...
		},
		{
			...
			message: 'TGM Plugin Activation is heavily restricted/prohibited by TRT.',
			severity: REQUIRED,
		},
	];
