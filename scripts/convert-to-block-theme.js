...
function removeMethodOnce(source, methodName) {
	...
	const endIndex = findBalancedEndSkippingLiterals(source, bodyStart, '{', '}');
	if (endIndex === -1) {
...
function removeThemeSupportLines(source, supportKeys) {
	...
	const fnMatch = /function\s+action_essential_theme_support\s*\(\)\s*\{/m.exec(
		source
	);
	if (!fnMatch) {
	...
	const endPos = findBalancedEndSkippingLiterals(source, startBrace, '{', '}');
	if (endPos === -1) {
		return { source, removed: [] };
	}
...
async function main() {
	...
	if (!/class\s+Component\b/.test(source) || source.trim().length < 100) {
		console.error(
...
async function processCustomizerComponent(dryRun) {
	...
	if (bounds) {
		...
		if (!/wp_is_block_theme\s*\(\s*\)/.test(body)) {
			...
			source = source.slice(0, bodyStart) + insertion + source.slice(bodyStart);
		}
	}
	...
	if (report.changed) {
		...
		if (!/class\s+Component\b/.test(source) || source.trim().length < 80) {
