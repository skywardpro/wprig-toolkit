...
function updateThemeComponents() {
	...
	try {
		...
		if (!componentsMethodMatch) {
			addLog('⚠️ Could not find get_default_components method in Theme.php');
			return;
		}
		...
		addLog(`🧹 Removed ${removedComponents.length} components from Theme.php`);
	} catch (e) {
...
function removeIncComponents(backupDir) {
	...
	try {
		...
		const dirsToMove = items.filter((item) => {
			...
			if (COMPONENTS_TO_KEEP.includes(item) || requiredFiles.includes(item)) {
				return false;
			}
