...
async function createRigComponent() {
	try {
		...
		console.log(`  Namespace: WP_Rig\\WP_Rig\\${componentInfo.pascalName}`);

		// Create directories
		const componentDir = path.join(themeRoot, 'inc', componentInfo.pascalName);
...
async function createComponentFile(filePath, componentInfo, options) {
	...
	if (templating) {
		useStatements.push(`use WP_Rig\\WP_Rig\\Templating_Component_Interface;`);
	}
...
async function wireComponent(componentInfo) {
	try {
		...
		if (await fileExists(functionsPath)) {
			...
			const functionsMatch = functionsContent.match(registerComponentsRegex);
		...
		console.warn(`Couldn't auto-wire the component. Please add manually:`);
		console.warn(`new ${componentInfo.pascalName}\\Component(),`);
		console.warn(
			`to the components array in inc/Theme.php or inc/functions.php`
		);
	} catch (error) {
		console.warn(`Warning: Failed to auto-wire component: ${error.message}`);
		console.warn(
