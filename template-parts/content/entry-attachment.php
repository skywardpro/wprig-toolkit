...
<article id="post-<?php the_ID(); ?>" <?php post_class('entry'); ?>>
	<?php get_template_part(
 	'template-parts/content/entry_header',
 	get_post_type(),
 ); ?>

	<?php get_template_part(
 	'template-parts/content/entry_content',
 	get_post_type(),
 ); ?>

	<?php get_template_part(
 	'template-parts/content/entry_footer',
 	get_post_type(),
 ); ?>
...
<?php if (is_singular(get_post_type())) {
	...
	if (!empty($post->post_parent)) {
		...
		if (!('' === $prev_link || '0' === $prev_link || false === $prev_link)) {
			...
			$attachment_navigation .=
				'<div class="post-navigation-sub"><span>' .
				esc_html__('Previous:', 'wp-rig') .
				'</span></div>';
