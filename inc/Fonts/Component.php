...
class Component implements Component_Interface, Templating_Component_Interface
{
	...
	{
		add_action('wp_enqueue_scripts', [$this, 'action_enqueue_fonts']);
		add_action('after_setup_theme', [$this, 'action_add_editor_fonts']);
		add_action('init', [$this, 'wprig_register_fonts']);
				...
				'font_families' => [
					[
						...
						'file' =>
							...
							'/assets/fonts/my-local-font/my-local-font.woff2',
						'weight' => '400',
						'style' => 'normal',
					],
				],
	...
	public function filter_resource_hints(
		...
		if (
			...
			wp_style_is('wp-rig-fonts', 'queue')
		) {
			$urls[] = [
	...
	protected function get_google_fonts(): array
	{
		...
		$google_fonts = [
			...
			'Montserrat' => ['100', '100i', '300', '500', '500i', '700', '700i'],
		];
		 ...
		 */
		$this->google_fonts = (array) apply_filters(
			'wp_rig_google_fonts',
			$google_fonts,
		);
	 ...
	 */
	public function download_all_google_fonts(
		string $font_dir = 'assets/fonts',
		string $css_dir = 'assets/css/src',
	) {
		...
		foreach (array_keys($google_fonts) as $font_name) {
			...
			$fonts_to_download[$font_name] = ['ital,wght@0,100..900'];
		}

		// Download and save all fonts locally in one go.
		if ([] !== $fonts_to_download) {
			return $this->download_google_fonts_to_local(
				$fonts_to_download,
				$font_dir,
				$css_dir,
			);
		...
		return new \WP_Error(
			'no_fonts_to_download',
			'No Google Fonts were found to download.',
		);
	}
	 ...
	 */
	public function download_google_fonts_to_local(
		$fonts = [],
		...
		$google_fonts_url =
			$google_fonts_base_url . implode('&', $query_fonts) . '&display=swap';
		...
		$response = wp_remote_get($google_fonts_url, [
			'headers' => [
				...
				'User-Agent' =>
					'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.5906.69 Safari/537.36',
			],
		]);
		...
		if (
			...
			return new \WP_Error(
				...
				'Could not fetch Google Fonts CSS.',
			);
		}
		...
		foreach ($fonts_with_urls as $font_name => $font_urls) {
			foreach ($font_urls as $font_url) {
				...
				if (!file_exists($local_font_path)) {
					...
					if (
						is_wp_error($font_response) ||
						wp_remote_retrieve_response_code($font_response) !== 200
					) {
					...
					$wp_filesystem->put_contents(
						$local_font_path,
						wp_remote_retrieve_body($font_response),
						FS_CHMOD_FILE,
					);
				}

				// Calculate the relative path from CSS directory to font directory.
				$css_to_font_relative_path = $this->get_relative_path(
					$css_dir,
					$font_dir,
				);
				...
				$relative_font_path =
					trailingslashit($css_to_font_relative_path) .
	 ...
	 */
	protected function get_local_font_path(
		string $font_name,
		string $variants = '400;700',
	) {
		...
		return file_exists($local_css_path)
			? trailingslashit($upload_dir['baseurl']) . 'fonts/' . $css_file_name
			: false;
	}
