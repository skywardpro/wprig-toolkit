...
function wp_rig_get_insufficient_requirements_message()
{
	...
	$insufficient_wp = version_compare(
		$wp_version,
		WP_RIG_MINIMUM_WP_VERSION,
		'<',
	);
	$insufficient_php = version_compare(
		phpversion(),
		WP_RIG_MINIMUM_PHP_VERSION,
		'<',
	);
	...
	if ($insufficient_wp && $insufficient_php) {
		...
		return sprintf(
			__(
				'WP Rig requires at least WordPress version %1$s and PHP version %2$s. You are running versions %3$s and %4$s respectively. Please update and try again.',
				'wp-rig',
			),
			WP_RIG_MINIMUM_WP_VERSION,
			WP_RIG_MINIMUM_PHP_VERSION,
	...
	if ($insufficient_wp) {
		...
		return sprintf(
			__(
				'WP Rig requires at least WordPress version %1$s. You are running version %2$s. Please update and try again.',
				'wp-rig',
			),
			WP_RIG_MINIMUM_WP_VERSION,
			$wp_version,
	...
	if ($insufficient_php) {
		...
		return sprintf(
			__(
				'WP Rig requires at least PHP version %1$s. You are running version %2$s. Please update and try again.',
				'wp-rig',
			),
			WP_RIG_MINIMUM_PHP_VERSION,
			phpversion(),
