	...
	public function action_activate_gsap()
	{
		...
		if (in_array('sto', $this->enabled_plugins)) {
			$this->enqueue_script(
				...
				'gsap/scroll-to-plugin.min.js',
				['gsap'],
			);
		}
		if (in_array('ce', $this->enabled_plugins)) {
			$this->enqueue_script('gsap-custom-easy', 'gsap/custom-easy.min.js', [
				'gsap',
			]);
		}
		...
		if (in_array('stext', $this->enabled_plugins)) {
			$this->enqueue_script('gsap-split-text', 'gsap/split-text.min.js', [
				'gsap',
			]);
		}
	}
