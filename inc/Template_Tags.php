...
class Template_Tags
{
	...
	/**
	 * Cached SVG icons from assets/svg directory.
	 *
	 * @var array|null
	 */
	private static $svg_cache = null;

	/**
	 * Gets all SVG icons from assets/svg directory with caching.
	 *
	 * @return array Associative array of filename => SVG content.
	 */
	public function get_all_svg_icons(): array
	{
		if (null !== self::$svg_cache) {
		...
		// Initialize WordPress Filesystem for reading file contents
		global $wp_filesystem;
		if (empty($wp_filesystem)) {
			require_once ABSPATH . '/wp-admin/includes/file.php';
			WP_Filesystem();
		...
		foreach ($svg_files as $file_path) {
			...
			if ($wp_filesystem && method_exists($wp_filesystem, 'get_contents')) {
				$content = $wp_filesystem->get_contents($file_path_normalized);
			} else {
	...
	/**
	 * Gets a specific SVG icon by name (without extension).
	 *
	 * @param string $icon_name Icon name without .svg extension.
	 * @return string|null SVG content or null if not found.
	 */
	public function get_svg_icon(string $icon_name): ?string
	{
		$icons = $this->get_all_svg_icons();
		$icon_name = strtolower($icon_name);

		// Icons are already stored in lowercase, so direct lookup
