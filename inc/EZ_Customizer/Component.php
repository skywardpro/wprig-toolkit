...
class Component implements Component_Interface
{
	...
	private function hooks()
	{
		add_action('customize_register', [
			$this,
			'ez_customizer_settings_register',
		]);
	}
	...
	private function add_settings()
	{
		foreach ($this->theme_settings['settings'] as $setting) {
			...
			if (!isset($setting['type'])) {
				$control = [
					'id' =>
						$this->theme_settings['theme_name'] . '_theme_' . $setting['id'],
					'label' => $setting['label'],
					'section' => ($args['section'] =
						$this->settings_id . '_' . $setting['section'] . '_section'),
				];
	...
	private function get_type_control(array $setting)
	{
		...
		switch ($setting['type']) {
			...
			case 'date':
				return new WP_Customize_Date_Time_Control(
					$this->wp_customize,
					$control_id,
					$args,
				);
