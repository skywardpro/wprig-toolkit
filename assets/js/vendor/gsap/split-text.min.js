	...
	let A,
		...
		ie = (e, t, s, i, o, c, n, a, h, b) => {
			...
			let g = Array.from(e.childNodes),
				...
				D =
					!L &&
					window.getComputedStyle(e).whiteSpace.substring(0, 3) ===
						"pre",
				E = 0,
				v = s.collection,
				r,
...
` &&
							...
							if (c && b) {
								if (
									((w = l.textContent =
										w.substring(p.length + 1, w.length)),
									(W = y.getBoundingClientRect()),
									W.top > j.top && W.left <= j.left)
								) {
									for (
										O = e.cloneNode(), T = e.childNodes[0];
										T && T !== y;
