...
class WP_Test_Setup
{
	...
	public function __construct()
	{
		...
		if (PHP_OS_FAMILY === 'Windows') {
			...
			$this->wp_tests_dir =
				getenv('WP_TESTS_DIR') ?: '/tmp/wordpress-tests-lib';
	...
	{
		if (
			is_dir($this->wp_core_dir) &&
			file_exists(
				$this->wp_core_dir . DIRECTORY_SEPARATOR . 'wp-config-sample.php',
			)
		) {
	...
	{
		$config_path =
			$this->wp_tests_dir . DIRECTORY_SEPARATOR . 'wp-tests-config.php';
	...
	private function get_wp_config_template()
	{
		...
		if (!is_dir($this->wp_core_dir)) {
			throw new Exception(
				"WordPress core directory does not exist: {$this->wp_core_dir}",
			);
		}
...
";
	...
	private function download_and_extract(
		$url,
		$extract_to,
	...
	private function extract_tar_gz(
		$tar_file,
		$extract_to,
		$expected_folder = null,
	) {
		...
		if (class_exists('PharData')) {
			try {
				...
				if ($expected_folder) {
					$extracted_path =
						$extract_to . DIRECTORY_SEPARATOR . $expected_folder;
	...
	private function move_directory_contents($source, $target)
	{
		...
		foreach ($iterator as $item) {
			$target_path =
				$target . DIRECTORY_SEPARATOR . $iterator->getSubPathName();
			if ($item->isDir()) {
...
try {
	...
	$test_dir =
		...
		DIRECTORY_SEPARATOR .
		'wordpress-tests-lib';
	$test_dir = str_replace(['/', '\\'], DIRECTORY_SEPARATOR, $test_dir);
